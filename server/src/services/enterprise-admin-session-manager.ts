...
const ADMIN_JWT_SECRET =
  ...
  process.env.VITE_ADMIN_JWT_SECRET ||
  (() => {
    if (process.env.NODE_ENV === "production") {
      throw new Error(
        "🚨 CRITICAL: ADMIN_JWT_SECRET environment variable is required in production"
      );
    }
    // Fixed: Generate consistent secret in development for session persistence across restarts
    const devSecret =
      process.env.DEV_ADMIN_JWT_SECRET ||
...
const COOKIE_DOMAIN =
  process.env.COOKIE_DOMAIN || process.env.VITE_COOKIE_DOMAIN;
...
export class EnterpriseAdminSessionManager {
  ...
  setSecureCookies(res: Response, session: AdminSession): void {
    ...
    res.cookie(
      "admin_session_id",
      this.extractSessionIdFromToken(session.token),
      {
        ...cookieOptions,
    ...
    res.cookie(
      "admin_fingerprint",
      session.metadata.fingerprint.substring(0, 16),
      {
        ...cookieOptions,
  ...
  private generateJWT(user: AdminUser, sessionId: string): string {
    const payload = {
      ...
      exp: Math.floor(
        (Date.now() + this.parseTimeToMs(SESSION_CONFIG.JWT_EXPIRY)) / 1000
      ),
    };
  ...
  private async recoverSessionFromDatabase(
    sessionId: string
    ...
    try {
      ...
      const logEntry = {
        ...
        metadata: JSON.stringify({
          ...
          ...(options.investigated !== undefined && {
  ...
  private async logSuspiciousActivity(
    ...
    setTimeout(
      () => this.suspiciousActivityLog.delete(activityKey),
      60 * 60 * 1000
    );
  }
...
export const enterpriseAdminSessionManager =
  new EnterpriseAdminSessionManager();
