...
export function parseId(value: any): number {
  ...
  throw new Error(`Invalid ID type: expected number or string, got ${typeof value}`);
}
 ...
 */
export function safeParseInt(value: any, fieldName: string, rowIndex?: number): number {
  try {
    return parseId(value);
  } catch (error) {
...
export function validateAndSanitizeInput(
  ...
  const { required = false, maxLength, minLength, allowedValues, regex } = options;
 ...
 */
export function validateArray(input: any, fieldName: string, itemValidator?: (item: any) => any): any[] {
