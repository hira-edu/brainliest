...
import { useSecuredAuth } from "./secured-auth-system";
...
function UnifiedAuthModalContent({
  ...
  const defaultTitle =
    mode === "freemium" ? "Unlock Unlimited Access" : "Welcome to Brainliest";
    ...
    async (action: string): Promise<string> => {
      if (!executeRecaptcha) {
        if (import.meta.env.MODE === "development") {
          console.warn(
            `🔒 executeRecaptcha function not available for action: ${action}`
          );
        }
        return "";
      }
      ...
      try {
        ...
        console.error(
          `🔒 reCAPTCHA execution failed for action ${action}:`,
          errorMessage
        );
        return "";
      }
        ...
        if (result.success) {
          if (result.requiresEmailVerification) {
            ...
            toast({
              title: "Account Created!",
              description:
                "Welcome to Brainliest! Your account has been created successfully.",
            });
          }
        } else {
          showErrorToast(
    ...
    resendDebounceRef.current = setTimeout(async () => {
      try {
        ...
        if (resendEmailVerification) {
          result = await resendEmailVerification(
            formData.email,
            recaptchaToken
          );
        } else {
