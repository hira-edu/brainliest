  ...
  return useQuery<UploadedIcon[]>({
    ...
    queryFn: async () => {
      const response = await fetch(
        "/api/admin/uploads?fileType=image&limit=100",
        {
          headers: {
      ...
      return (
        data.uploads?.filter((upload: UploadedIcon) => upload.isActive) || []
      );
    },
  });
}
...
function IconSelector({
  ...
  const iconOptions = useMemo(() => {
    ...
    // Add "No Icon" option
    return [{ value: "", label: "No Icon", disabled: false }, ...options];
  }, [icons]);
...
export default function AdminSimple() {
  const { user: adminUser, signOut: logout } = useSecuredAuth();
  ...
  const handleUnifiedCsvImport = async (
      ...
      try {
        ...
        if (result.success) {
          ...
          toast({
            ...
            description: `Platform data imported: ${
              result.subjects || 0
            } subjects, ${result.exams || 0} exams, ${
              result.questions || 0
            } questions`,
          });
  ...
  const [selectedSubjectForExport, setSelectedSubjectForExport] =
    useState<string>("");
  ...
  const [selectedExamCategoryFilter, setSelectedExamCategoryFilter] = useState<
    string | null
  >(null);
  const [selectedExamSubcategoryFilter, setSelectedExamSubcategoryFilter] =
  ...
  function DashboardOverview() {
    ...
    const questionsBySubject =
      subjects?.map((subject) => ({
        name: subject.name,
        count: questionsList.filter((q) => q.subjectSlug === subject.slug)
          .length,
      })) || [];
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-600">
                Active certification categories
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-xl font-bold">
                {popularExam?.title?.substring(0, 15) || "No Data"}
              </div>
              <p className="text-xs text-gray-600">
                {popularExam?.count || 0} questions
              </p>
            </CardContent>
          </Card>
        </div>
  ...
  function CategoryManager() {
    ...
    const deleteCategoryMutation = useMutation({
      mutationFn: (slug: string) =>
        apiRequest("DELETE", `/api/categories/${slug}`),
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["/api/categories"] });
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Create and organize main subject categories
            </p>
          </div>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent>
            ...
            <Form {...editCategoryForm}>
              <form
                onSubmit={editCategoryForm.handleSubmit(onEditSubmit)}
                className="space-y-4"
              >
                <FormField
  ...
  function SubcategoryManager() {
    ...
    const deleteSubcategoryMutation = useMutation({
      mutationFn: (slug: string) =>
        apiRequest("DELETE", `/api/subcategories/${slug}`),
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ["/api/subcategories"] });
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Create subcategories within main categories
            </p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
            ...
            <DialogContent>
              ...
              <Form {...subcategoryForm}>
                <form
                  onSubmit={subcategoryForm.handleSubmit(onSubmit)}
                  className="space-y-4"
                >
                  <FormField
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g., IT & Cloud Computing"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          Parent: {parentCategory?.name} | Sort Order:{" "}
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          deleteSubcategoryMutation.mutate(subcategory.slug)
                        }
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent>
            ...
            <Form {...editSubcategoryForm}>
              <form
                onSubmit={editSubcategoryForm.handleSubmit(onEditSubmit)}
                className="space-y-4"
              >
                <FormField
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Create and organize certification categories
            </p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <div className="text-xs text-gray-600 mt-1">
                          💡 For automatic categorization, include keywords
                          like: "certification", "aws", "pmp", "statistics",
                              ...
                              <div className="space-y-4">
                                ...
                                <Button
                                  onClick={() => {
                                    ...
                                    const name = (
                                      document.getElementById(
                                        "quick-subcategory-name"
                                      ) as HTMLInputElement
                                    )?.value;
                                    const description = (
                                      document.getElementById(
                                        "quick-subcategory-desc"
                                      ) as HTMLTextAreaElement
                                    )?.value;
                                    const icon = (
                                      document.getElementById(
                                        "quick-subcategory-icon"
                                      ) as HTMLInputElement
                                    )?.value;
          ...
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            <DialogContent>
              ...
              <Form {...editSubjectForm}>
                <form
                  onSubmit={editSubjectForm.handleSubmit(onEditSubmit)}
                  className="space-y-4"
                >
                  <FormField
                          ...
                          <Dialog>
                            ...
                            <DialogContent>
                              ...
                              <div className="space-y-4">
                                <Input
                                  placeholder="Category name"
                                  id="edit-quick-category-name"
                                />
                                <Textarea
                                  placeholder="Description (optional)"
                                  id="edit-quick-category-desc"
                                />
                                <Input
                                  placeholder="Icon (optional)"
                                  id="edit-quick-category-icon"
                                />
                                <Button
                                  onClick={() => {
                                    const name = (
                                      document.getElementById(
                                        "edit-quick-category-name"
                                      ) as HTMLInputElement
                                    )?.value;
                                    const description = (
                                      document.getElementById(
                                        "edit-quick-category-desc"
                                      ) as HTMLTextAreaElement
                                    )?.value;
                                    const icon = (
                                      document.getElementById(
                                        "edit-quick-category-icon"
                                      ) as HTMLInputElement
                                    )?.value;
                                    if (name) {
                                      createCategoryMutation.mutate({
                          ...
                          <Dialog>
                            ...
                            <DialogContent>
                              ...
                              <div className="space-y-4">
                                ...
                                <input
                                  type="hidden"
                                  id="edit-quick-subcategory-parent"
                                />
                                <Input
                                  placeholder="Subcategory name"
                                  id="edit-quick-subcategory-name"
                                />
                                <Textarea
                                  placeholder="Description (optional)"
                                  id="edit-quick-subcategory-desc"
                                />
                                <Input
                                  placeholder="Icon (optional)"
                                  id="edit-quick-subcategory-icon"
                                />
                                <Button
                                  onClick={() => {
                                    const categorySlug = (
                                    ...
                                    const name = (
                                      document.getElementById(
                                        "edit-quick-subcategory-name"
                                      ) as HTMLInputElement
                                    )?.value;
                                    const description = (
                                      document.getElementById(
                                        "edit-quick-subcategory-desc"
                                      ) as HTMLTextAreaElement
                                    )?.value;
                                    const icon = (
                                      document.getElementById(
                                        "edit-quick-subcategory-icon"
                                      ) as HTMLInputElement
                                    )?.value;
                                    if (categorySlug && name) {
                                      createSubcategoryMutation.mutate({
                  ...
                  ?.filter(
                    (subcategory) =>
                      ...
                      (!selectedCategoryFilter ||
                        subcategory.categorySlug === selectedCategoryFilter)
                  )
                  ?.map((subcategory) => ({
                    value: subcategory.slug,
                    label: subcategory.name,
            ...
            <Badge variant="outline">
              {subjects?.filter((subject) => {
                const matchesCategory =
                  !selectedCategoryFilter ||
                  subject.categorySlug === selectedCategoryFilter;
                const matchesSubcategory =
                  !selectedSubcategoryFilter ||
                  subject.subcategorySlug === selectedSubcategoryFilter;
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center justify-between flex-1 mr-4">
                      ...
                      <div
                        className="w-8 h-8 ml-4 flex items-center justify-center"
                        title={subject.icon || "Default icon"}
                      >
                        <DynamicIcon
                          name={subject.icon}
                    ...
                    <div className="flex items-center space-x-2 flex-shrink-0">
                      <Badge variant="outline">
                        {exams?.filter((e) => e.subjectSlug === subject.slug)
                          .length || 0}{" "}
                        exams
                      </Badge>
                      <Badge variant="outline">
                        {questions?.filter(
                          (q) => q.subjectSlug === subject.slug
                        ).length || 0}{" "}
                        questions
                      </Badge>
                      <Button
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          deleteSubjectMutation.mutate(subject.slug)
                        }
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Create, clone, and organize practice exams
            </p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g., PMP Practice Exam 1"
                            {...field}
                            value={field.value || ""}
                          />
                        </FormControl>
          ...
          <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
            <DialogContent className="max-w-2xl">
              ...
              <Form {...editExamForm}>
                <form
                  onSubmit={editExamForm.handleSubmit(onEditSubmit)}
                  className="space-y-4"
                >
                  <FormField
        ...
        <div className="mb-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <SearchableSelect
                ...
                ...(subcategories
                  ?.filter(
                    (subcategory) =>
                      subcategory?.slug &&
                      subcategory?.name &&
                      (!selectedExamCategoryFilter ||
                        subcategory.categorySlug === selectedExamCategoryFilter)
                  )
                  ?.map((subcategory) => ({
                    value: subcategory.slug,
                    label: subcategory.name,
              ...
              onValueChange={(value) => {
                setSelectedExamSubcategoryFilter(
            ...
            <SearchableSelect
              options={[
                ...
                ...(subjects
                  ?.filter((subject) => {
                    ...
                    const matchesCategory =
                      !selectedExamCategoryFilter ||
                      subject.categorySlug === selectedExamCategoryFilter;
                    const matchesSubcategory =
                      !selectedExamSubcategoryFilter ||
                      subject.subcategorySlug === selectedExamSubcategoryFilter;
                    return matchesCategory && matchesSubcategory;
                  })
                  .map((subject) => ({
                    value: subject.slug,
              ...
              emptyText="No subjects found"
              disabled={
                !selectedExamCategoryFilter && !selectedExamSubcategoryFilter
              }
              className="w-48"
              clearable
            />
            ...
            <Badge variant="outline">
              {exams?.filter((exam) => {
                ...
                const matchesSubject =
                  selectedSubjectFilter === "all" ||
                  exam.subjectSlug === selectedSubjectFilter;
        ...
        <div className="grid gap-4">
          {exams
            ...
            .map((exam) => {
              ...
              const questionCount =
                questions?.filter((q) => q.examSlug === exam.slug).length || 0;
  ...
  function QuestionManager() {
    ...
    const filteredQuestions =
      questions?.filter((question) => {
        ...
        const matchesSubject =
          selectedSubject === "all" || question.subjectSlug === selectedSubject;

        const matchesExam =
          selectedExam === "all" || question.examSlug === selectedExam;
    ...
    const createQuestionMutation = useMutation({
      mutationFn: async (data: QuestionFormData) => {
        ...
        const cleanOptions = data.options.filter(
          (option) => option.trim() !== ""
        );
        const questionData: InsertQuestion = {
          ...data,
          options: cleanOptions,
          correctAnswers: data.allowMultipleAnswers
            ? data.correctAnswers
            : null,
        };
      ...
      <div className="space-y-6">
            ...
            <div className="flex gap-2">
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3">
                            <div className="space-y-0.5">
                              ...
                              <FormDescription>
                                Allow users to select more than one correct
                                answer
                              </FormDescription>
                            </div>
                            <FormControl>
              ...
              >
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                  ...
                  <Form {...questionForm}>
                    <form
                      onSubmit={questionForm.handleSubmit(onEditSubmit)}
                      className="space-y-4"
                    >
                      <FormField
                        control={questionForm.control}
              ...
              options={[
                {
                  ...
                  label:
                    selectedSubject === "all"
                      ? "All Exams"
                      : "All Exams in Subject",
                },
                ...(selectedSubject === "all"
                  ? exams
                      ?.filter((exam) => exam?.slug && exam?.title)
                      .map((exam) => ({
                        value: exam.slug,
                        label: exam.title,
                      })) || []
                  : exams
                      ?.filter(
                        (exam) =>
                          exam.subjectSlug === selectedSubject &&
                          exam?.slug &&
                          exam?.title
                      )
                      ?.map((exam) => ({
        ...
        {importStats && (
          <Alert>
            ...
            <AlertDescription>
              Successfully imported {importStats.created} out of{" "}
              {importStats.total} questions from CSV file.
            </AlertDescription>
          </Alert>
        )}
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Manage questions, exams, and subjects with CSV import/export
                functionality
              </p>
            </div>
            <Button
        ...
        <Tabs defaultValue="dashboard" className="space-y-8">
          <TabsList className="grid w-full grid-cols-6 h-12 p-1">
            <TabsTrigger
              value="dashboard"
              className="text-sm font-medium py-3 px-6"
            >
              Dashboard
            </TabsTrigger>
            <TabsTrigger
            ...
            <TabsTrigger value="csv" className="text-sm font-medium py-3 px-6">
              Import/Export
            </TabsTrigger>
          </TabsList>
            ...
            <div>
              <Tabs defaultValue="overview" className="space-y-6">
                <TabsList className="grid w-full grid-cols-7 h-11 p-1 bg-gray-100">
                  <TabsTrigger
                    value="overview"
                    className="text-xs font-medium py-2 px-4"
                  >
                    Overview
                  </TabsTrigger>
                  <TabsTrigger
          ...
          <TabsContent value="csv" className="space-y-8">
            <Card>
              <CardHeader>
                ...
                <p className="text-gray-600">
                  Manage data with comprehensive CSV import and export
                  functionality
                </p>
              </CardHeader>
              <CardContent>
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <Card className="p-6 border-2 border-dashed border-blue-200 hover:border-blue-400 transition-colors">
                        ...
                        <p className="text-sm text-gray-600 mb-4">
                          Get the unified CSV template for adding all types of
                          ...
                          onClick={async () => {
                            try {
                              ...
                              if (response.ok) {
                                ...
                                link.href = url;
                                link.download =
                                  "brainliest_complete_platform_template.csv";
                                link.click();
                                window.URL.revokeObjectURL(url);
                                toast({
                                  title: "Complete template downloaded",
                                  description:
                                    "Unified platform template with all entities ready for editing",
                                });
                              } else {
                                toast({
                      ...
                      <Card className="p-6 border-2 border-dashed border-green-200 hover:border-green-400 transition-colors">
                        ...
                        <p className="text-sm text-gray-600 mb-4">
                          Export all current data with proper formatting and
                          ...
                          onClick={async () => {
                            try {
                              ...
                              if (response.ok) {
                                ...
                                const link = document.createElement("a");
                                link.href = url;
                                link.download = `brainliest_complete_platform_export_${
                                ...
                                toast({
                                  ...
                                  description:
                                    "Complete platform data exported successfully",
                                });
                              } else {
                                toast({
                                  title: "Export failed",
                      ...
                      <Card className="p-6 border-2 border-dashed border-purple-200 hover:border-purple-400 transition-colors">
                        ...
                        <p className="text-sm text-gray-600 mb-4">
                          Upload your completed CSV file to add content with
                  ...
                  <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
                    <h4 className="font-medium mb-2">
                      Complete Platform CSV Format
                    </h4>
                    <div className="text-sm text-gray-600 dark:text-gray-300 space-y-2">
                      <div>
                        <p className="font-semibold">
                          Row Structure (All entities in one file):
                        </p>
                        <p>
                          • <strong>Entity Type</strong> (subject|exam|question)
                          - First column determines row type
                        </p>
                        <p>
                          • <strong>Subject columns:</strong> name, description,
                          icon, sortOrder, categorySlug, subcategorySlug
                        </p>
                        <p>
                          • <strong>Exam columns:</strong> title, description,
                          subjectSlug, difficulty, timeLimit, passingScore
                        </p>
                        <p>
                          • <strong>Question columns:</strong> text, options
                          (pipe-separated), correctAnswer, explanation,
                          examSlug, subjectSlug, domain
                        </p>
                      </div>
                      <div>
                        <p className="font-semibold">
                        ...
                        <p>
                          • Questions link to exams and subjects by name or ID
                        </p>
                ...
                <div className="mt-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <Card className="p-6 border-2 border-dashed border-green-200 hover:border-green-400 transition-colors">
                      ...
                      <p className="text-sm text-gray-600 mb-4">
                        Get structured JSON template for hierarchical data
                        import with all fields and validation.
                      </p>
                      <Button
                        onClick={async () => {
                          try {
                            ...
                            if (response.ok) {
                              ...
                              toast({
                                title: "JSON template downloaded successfully!",
                              });
                            } else {
                              toast({
                                title: "Failed to download JSON template",
                                variant: "destructive",
                              });
                            }
                          } catch (error) {
                            toast({
                              title: "Error downloading JSON template",
                              variant: "destructive",
                            });
                          }
                        }}
                    ...
                    <Card className="p-6 border-2 border-dashed border-orange-200 hover:border-orange-400 transition-colors">
                      ...
                      <p className="text-sm text-gray-600 mb-4">
                        Export existing data as structured JSON format for
                        backup or migration.
                      </p>
                      ...
                      <div className="space-y-3">
                        <div>
                          <label className="block text-sm font-medium mb-2">
                            Select Subject to Export
                          </label>
                          <Select
                            value={selectedSubjectForExport}
                            onValueChange={setSelectedSubjectForExport}
                          >
                        ...
                        <Button
                          onClick={async () => {
                            if (!selectedSubjectForExport) {
                              toast({
                                title: "Please select a subject to export",
                                variant: "destructive",
                              });
                              return;
                            }
                            ...
                            try {
                              ...
                              const response = await fetch(
                                `/api/json/export/${selectedSubjectForExport}`,
                                {
                                  method: "GET",
                                  headers: {
                                    Authorization: `Bearer ${adminToken}`,
                              ...
                              if (response.ok) {
                                ...
                                const a = document.createElement("a");
                                a.href = url;
                                a.download = `subject_${selectedSubjectForExport}_export_${
                                  new Date().toISOString().split("T")[0]
                                }.json`;
                                document.body.appendChild(a);
                                a.click();
                                ...
                                document.body.removeChild(a);
                                toast({
                                  title: "Data exported to JSON successfully!",
                                });
                              } else {
                                toast({
                    ...
                    <Card className="p-6 border-2 border-dashed border-indigo-200 hover:border-indigo-400 transition-colors">
                      ...
                      <p className="text-sm text-gray-600 mb-4">
                        Upload JSON file to import structured data with
              ...
              <CardContent>
                <div className="text-center py-8">
                  ...
                  <p className="mt-4 text-gray-500">
                    Detailed question performance, user stats, and exam
